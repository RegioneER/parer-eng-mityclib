...
public class TSCliente {
    ...
    /** Looger. */
    static Logger log = LoggerFactory.getLogger(TSCliente.class.getName());
    ...
    public TSCliente(final String nombreServidor, final String algoritmoHash) {
        ...
        // Comprueba que el algoritmo configurado en propiedades es aceptado. Si no lo es deja el
    ...
    public static void setSSLManager(ISSLManager sslmanager) {
        Protocol authhttps = new Protocol("https", new OwnSSLProtocolSocketFactory(sslmanager),
                443);
    ...
    public byte[] generarSelloTiempo(final byte[] binarioaSellar) throws TSClienteError {
        if (binarioaSellar == null) {
            ...
            try {
                ...
                peticion = generadorPeticion.generate(TSPAlgoritmos.getOID(algoritmoHash),
                        resumen.digest());
            ...
            if (servidorProxy != null) {
                ...
                Credentials defaultcreds = new AuthenticatorProxyCredentials(servidorProxy,
                        ConstantesTSA.CADENA_VACIA);
                CLIENTE.getState().setProxyCredentials(AuthScope.ANY, defaultcreds);
            }

            PostMethod metodo = new PostMethod(servidorTSA);
            metodo.addRequestHeader(ConstantesTSA.CONTENT_TYPE,
                    ConstantesTSA.APPLICATION_TIMESTAMP_QUERY);
            ...
            try {
                ...
                if (estadoCodigo != HttpStatus.SC_OK) {
                    log.error(
                            ConstantesTSA.MENSAJE_FALLO_EJECUCION_METODO + metodo.getStatusLine());
                ...
                try {
                    ...
                    try {
                        ...
                        ASN1Encodable enc = seq.getObjectAt(1);
                        if (enc == null) {
                            return null;
                        }
                        return enc.toASN1Primitive().getEncoded();
                        ...
                        throw new TSClienteError(
                                I18n.getResource(ConstantesTSA.LIBRERIA_TSA_ERROR_9)
                                        + ConstantesTSA.DOS_PUNTOS_ESPACIO + e.getMessage());
                    }
                } catch (TSPException e) {
                    ...
                    log.error(
                            ConstantesTSA.MENSAJE_SECUENCIA_BYTES_MAL_CODIFICADA + e.getMessage());
    ...
    public static void main(final String[] args) {
        // TSCliente cliente = new TSCliente("http://minister-6vp1kq.mityc.age:9207","SHA-1", null,
        // 0);
        // byte[] firma =
        // Base64.decode(("X1MlQzZRNqBeJR2hjunePlD+ywlkdgaBAo3QDRhItXGhb1k4FffA6V2w5KZoSjPCaDhMgwcTXxz3"+
