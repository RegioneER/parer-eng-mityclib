...
/**
 * A <code>SecureProtocolSocketFactory</code> that uses JSSE to create SSL sockets. It will also support host name
 * verification to help preventing man-in-the-middle attacks. Host name verification is turned <b>on</b> by default but
 * one will be able to turn it off, which might be a useful feature during development. Host name verification will make
 * sure the SSL sessions server host name matches with the the host name returned in the server certificates "Common
 * Name" field of the "SubjectDN" entry.
 *
...
public class OwnSSLProtocolSocketFactory implements SecureProtocolSocketFactory {
     ...
     *            443);
     *
     *            HttpClient client = new HttpClient(); client.getHostConfiguration().setHost("localhost", 443,
    ...
    /**
     * Parses a X.500 distinguished name for the value of the "Common Name" field. This is done a bit sloppy right now
     * and should probably be done a bit more according to <code>RFC 2253</code>.
     *
     * @param dn
