 ...
 *
 * <p>
 * Implementa la interfaz {@link IDecoder} para permitir su uso en <i>streams</i> de entrada,
 * pudiendo decodificar al vuelo en pequeños bloques de lectura.
 * </p>
 *
 * <p>
 * Hay que tener en cuenta que base64 se dispone en bloques de 4 elementos. Debido a que el bloque
 * obtenido para decodificar puede no ajustarse a ese tamaño, el decodificador mantiene un pequeño
 * buffer para los elementos que quedan fuera del bloque. Una vez finalizada la lectura y
 * decodificacion se aconseja comprobar mediante el método {@link #isIncomplete()} la existencia de
 * datos que no se han podido decodificar (es decir, que la trama base64 no finalizaba
 * correctamente).
 * </p>
 *
...
public class Base64Decoder implements IDecoder {
    ...
    /** Flag para el indicador de funcionar solo en modo de cuenta. */
     ...
     * </p>
     *
     * @param onlyCountMode Indica si se debe iniciar el decoder solo contando bytes sin decodificar
     */
     ...
     * @param pos  posicion del array desde la que se incluyen los datos
    ...
    /**
     * <p>
     * Pasa de Base64 a binario los datos disponibles en el buffer.
     * </p>
     * <p>
     * Lanzara una excepcion {@link ArrayIndexOutOfBoundsException} si los datos de acceso al buffer
     * de escritura son incorrectos.
     * </p>
     *
     * @param data Array en el que escribir los datos
     * @param off  Posicion a partir de la cual escribir
     * @param len  Cantidad de bytes maximo que se puede escribir
     *
    ...
    public int decode(final byte[] data, final int off, final int len) throws DecodingException {
        ...
        if (len < BASE64_CHUNK_DECODED_LENGTH) {
            throw new DecodingException(
                    "Buffer too small, minimum size = " + BASE64_CHUNK_DECODED_LENGTH);
        }
        int res = 0;
        if (buffer != null) {
            // Recoge lo que hay en el buffer y elimina caracteres anomalos
            ...
            int actualLen = len;
            if (actualLen < len64) {
                actualLen = actualLen - (actualLen % BASE64_CHUNK_DECODED_LENGTH); // Se queda con
                // multiplos de 3
                buffer = s
                        .substring((actualLen * BASE64_CHUNK_LENGTH) / BASE64_CHUNK_DECODED_LENGTH)
                        .getBytes();
                s = s.substring(0,
                        ((actualLen * BASE64_CHUNK_LENGTH) / BASE64_CHUNK_DECODED_LENGTH) - 1);
            } else {
                buffer = null;
            }
            if (!countMode) {
     ...
     * </p>
     *
     * @return <code>true</code> si quedan bytes sin haber sido decoficados en el buffer,
