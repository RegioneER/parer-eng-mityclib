...
public class UtilidadXadesX {
    ...
    /**
     * Obtiene el listado de nodos necesarios para el calculo del sello de tiempo para XAdES X del
     * tipo 1 (implícito) en funcion del esquema XAdES de firma, a partir del nodo SigAndRefs
     * proporcionado, que actúa como tope Los nodos necesarios son los siguientes: - SignatureValue
     * - SignatureTimestamp - CompleteCertificateRefs - CompleteRevocationRefs Opcionalmente en el
     ...
     *
     * @return ArrayList<Element> Coleccion de nodos para el calculo del sello de tiempo de tipo 1
     *         (implícito) SigAndRefsTimeStamp
     *
     * @throws BadFormedSignatureException Si no existe, o existe mas de un nodo, o no se
     *                                     corresponde con el esquema, según sea el caso
	...
	if (!(new NombreNodo(esquemaXADES, ConstantesXADES.UNSIGNED_SIGNATURE_PROPERTIES)
		.equals(new NombreNodo(UnsignedSignaturePropertiesElement.getNamespaceURI(),
			UnsignedSignaturePropertiesElement.getLocalName())))) {
	    // El nodo padre de SigAndRefsTimeStamp no es el nodo UnsignedSignatureProperties
	    // esperado según el esquema
	    // xades
	    throw new BadFormedSignatureException(
		    I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_40)
			    + ConstantesXADES.ESPACIO + ConstantesXADES.SIG_AND_REFS_TIME_STAMP
			    + ConstantesXADES.ESPACIO
	...
	if (ConstantesXADES.SCHEMA_XADES_122.equals(esquemaXADES)
		|| ConstantesXADES.SCHEMA_XADES_111.equals(esquemaXADES)) { // Búsqueda para los
									    // esquemas 1.1.1 y
									    // 1.2.2
									    // (coleccion de nodos
									    // ordenada)
	    ...
	    ArrayList<Element> nodosSigTimeStamp = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, tagName);
	    ...
	    ArrayList<Element> nodosCompleteCertificateRefs = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, tagName);

	    if (!(nodosCompleteCertificateRefs.size() == 1))
		// El nodo CompleteCertificateRefs no existe o no es único. Número de nodos
		// encontrados:
		throw new BadFormedSignatureException(
			I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_36)
				+ ConstantesXADES.ESPACIO
				...
				+ ConstantesXADES.ESPACIO
				+ I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_37)
				+ ConstantesXADES.ESPACIO + nodosCompleteCertificateRefs.size());
	    ...
	    ArrayList<Element> nodosCompleteRevocationRefs = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, tagName);

	    if (!(nodosCompleteRevocationRefs.size() == 1))
		// El nodo CompleteRevocationRefs no existe o no es único. Número de nodos
		// encontrados:
		throw new BadFormedSignatureException(
			I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_36)
				...
				+ ConstantesXADES.ESPACIO
				+ I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_37)
				+ ConstantesXADES.ESPACIO + nodosCompleteRevocationRefs.size());
		...
		ArrayList<Element> nodosAttributeCertificateRefs = UtilidadTratarNodo
			.obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, tagName);
		...
		if ((nodosAttributeCertificateRefs.size() > 1))
		    ...
		    throw new BadFormedSignatureException(
			    I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_36)
				    + ConstantesXADES.ESPACIO
				    + ConstantesXADES.ATTRIBUTE_CERTIFICATE_REFS
				    + ConstantesXADES.ESPACIO
				    + I18n.getResource(
					    ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_38)
				    + ConstantesXADES.ESPACIO
				    + nodosAttributeCertificateRefs.size());

		// Si se encontro el nodo, se agrega al array
		if ((nodosAttributeCertificateRefs.size() == 1))
		...
		ArrayList<Element> nodosAttributeRevocationRefs = UtilidadTratarNodo
			.obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, tagName);
		...
		if ((nodosAttributeRevocationRefs.size() > 1))
		    ...
		    throw new BadFormedSignatureException(
			    I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_36)
				    + ConstantesXADES.ESPACIO
				    + ConstantesXADES.ATTRIBUTE_REVOCATION_REFS
				    + ConstantesXADES.ESPACIO
				    + I18n.getResource(
					    ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_38)
				    + ConstantesXADES.ESPACIO
	    ...
	    nodosABuscar
		    .add(new NombreNodo(esquemaXADES, ConstantesXADES.COMPLETE_CERTIFICATE_REFS));
	    // CompleteRevocationRefs
	    nodosABuscar
		    .add(new NombreNodo(esquemaXADES, ConstantesXADES.COMPLETE_REVOCATION_REFS));
	    // Los nodos AttributeCertificateRefs y AttributeRevocationRefs son opcionales
	    nodosABuscar
		    .add(new NombreNodo(esquemaXADES, ConstantesXADES.ATTRIBUTE_CERTIFICATE_REFS));
	    nodosABuscar
		    .add(new NombreNodo(esquemaXADES, ConstantesXADES.ATTRIBUTE_REVOCATION_REFS));

	    // Se realiza la búsqueda
	    ArrayList<Element> nodos = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoSigAndRefs, nodosABuscar);
	...
	while (it.hasNext()) {
		...
		// Si el valor de index es < indexAnterior => el orden es invalido
		if (index < indexAnterior) {
		    // La cadena de nodos para el sello de tiempo no es valida
		    throw new BadFormedSignatureException(
			    I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_39));
		} else
		    indexAnterior = index;
	    }
    ...
    /**
     * Obtiene el listado de nodos necesarios para el calculo del sello de tiempo para XAdES X del
     * tipo 2 (explícito) en funcion del esquema XAdES de firma, a partir del nodo RefsOnly
     * proporcionado, que actúa como tope Los nodos necesarios son los siguientes: -
	...
	if (!(new NombreNodo(esquemaXADES, ConstantesXADES.UNSIGNED_SIGNATURE_PROPERTIES)
		.equals(new NombreNodo(UnsignedSignaturePropertiesElement.getNamespaceURI(),
			UnsignedSignaturePropertiesElement.getLocalName())))) {
	    // El nodo padre de RefsOnlyTimeStamp no es el nodo UnsignedSignatureProperties esperado
	    // según el esquema
	    // xades
	    throw new BadFormedSignatureException(
		    I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_40)
			    + ConstantesXADES.ESPACIO + ConstantesXADES.REFS_ONLY_TIME_STAMP
			    + ConstantesXADES.ESPACIO
			    + I18n.getResource(ConstantesXADES.LIBRERIAXADES_FIRMAXML_ERROR_42));
	}
	...
	if (ConstantesXADES.SCHEMA_XADES_132.equals(esquemaXADES)
	    ...
	    // Se agrega el nodo CompleteCertificateRefs
	    NombreNodo tagName = new NombreNodo(esquemaXADES,
		    ConstantesXADES.COMPLETE_CERTIFICATE_REFS);
	    ArrayList<Element> nodosCompleteCertificateRefs = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoRefsOnly, tagName);
	    ...
	    // Se realiza la búsqueda
	    ArrayList<Element> nodos = UtilidadTratarNodo
		    .obtenerNodos(UnsignedSignaturePropertiesElement, nodoRefsOnly, nodosABuscar);
