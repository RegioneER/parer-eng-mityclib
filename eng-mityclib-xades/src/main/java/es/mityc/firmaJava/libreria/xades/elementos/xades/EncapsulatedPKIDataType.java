...
public class EncapsulatedPKIDataType extends AbstractXADESElement {
    ...
    public void load(Element element) throws InvalidInfoNodeException {
        ...
        if (element.hasAttribute(ConstantesXADES.XADES_TAG_ENCODING)) {
            ...
            this.encoding = EncodingEnum
                    .getEncoding(element.getAttribute(ConstantesXADES.XADES_TAG_ENCODING));
            if (this.encoding == null)
                throw new InvalidInfoNodeException(
                        "Encoding de nodo EncapsulatedPKIDataType desconocido: "
        ...
        if (node.getNodeType() != Node.TEXT_NODE)
            throw new InvalidInfoNodeException(
                    "Nodo EncapsulatedPKIDataType no contiene CDATA como primer valor");

        this.value = node.getNodeValue();
        if (this.value == null)
            throw new InvalidInfoNodeException(
                    "Contenido de valor de EncapsulatedPKIDataType vacío");
        // TODO: chequear que es un contenido del tipo base64binary (en el encoding adecuado si
        // viene indicado).
    ...
    protected void addContent(Element element) throws InvalidInfoNodeException {
        if (value == null)
            throw new InvalidInfoNodeException(
                    "Informacion insuficiente para escribir nodo EncapsulatedPKIDataType");
        if (id != null)
        ...
        if ((!schema.equals(XAdESSchemas.XAdES_111)) && (encoding != null))
            element.setAttributeNS(null, ConstantesXADES.XADES_TAG_ENCODING,
                    encoding.getEncodingUri().toString());
