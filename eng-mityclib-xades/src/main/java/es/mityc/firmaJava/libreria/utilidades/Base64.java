...
public final class Base64 { // implements ConstantesXADES
	...
	if ((options & ConstantesXADES.URL_SAFE) == ConstantesXADES.URL_SAFE)
	    return _URL_SAFE_ALPHABET.clone();
	else if ((options & ConstantesXADES.ORDERED) == ConstantesXADES.ORDERED)
	...
	// Example: DkLE
		...
		System.out.println(ConstantesXADES.CADENA_VACIA + source[srcOffset]
			+ ConstantesXADES.DOS_PUNTOS_ESPACIO + (DECODABET[source[srcOffset]]));
		System.out.println(ConstantesXADES.CADENA_VACIA + source[srcOffset + 1]
			+ ConstantesXADES.DOS_PUNTOS_ESPACIO + (DECODABET[source[srcOffset + 1]]));
		System.out.println(ConstantesXADES.CADENA_VACIA + source[srcOffset + 2]
			+ ConstantesXADES.DOS_PUNTOS_ESPACIO + (DECODABET[source[srcOffset + 2]]));
		System.out.println(ConstantesXADES.CADENA_VACIA + source[srcOffset + 3]
			+ ConstantesXADES.DOS_PUNTOS_ESPACIO + (DECODABET[source[srcOffset + 3]]));
		return -1;
	    } // end catch
	}
    } // end decodeToBytes
	...
	try {
	    ...
	    log.error("Generic ClassNotFoundException", e);
	    obj = null;
	} // end catch
	finally {
	    try {
	...
	try {
	    ...
	    bos = new Base64.OutputStream(new FileOutputStream(filename), ConstantesXADES.ENCODE);
	...
	BufferedOutputStream bfos = null;
	try {
	    bos = new Base64.OutputStream(new FileOutputStream(filename), ConstantesXADES.DECODE);
	    bfos = new BufferedOutputStream(bos);
	...
	try {
		...
		log.error(ConstantesXADES.STR_FILE_TOO_BIG + file.length()
			+ ConstantesXADES.STR_BYTES);
		return null;
	...
	try {
	    ...
	    log.error("Generic IOException", ex);
	} // end catch
	finally {
	    try {
		out.close();
	...
	public InputStream(java.io.InputStream in, int options) {
	    ...
	    this.breakLines = (options
		    & ConstantesXADES.DONT_BREAK_LINES) != ConstantesXADES.DONT_BREAK_LINES;
